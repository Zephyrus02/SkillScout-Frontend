...
const SOCIAL_META: {
  key: "linkedin" | "github" | "twitter" | "website";
  label: string;
  ...
  },
  {
    ...
    icon: (
      <span className="w-6 h-6 flex items-center justify-center rounded-md flex-shrink-0 bg-gray-900 dark:bg-gray-100">
        <svg
          viewBox="0 0 24 24"
          className="w-3.5 h-3.5 fill-white dark:fill-gray-900"
        >
  ...
  },
  {
    ...
    icon: (
      <span className="w-6 h-6 flex items-center justify-center rounded-md flex-shrink-0 bg-black dark:bg-white">
        <svg
          viewBox="0 0 24 24"
          className="w-3.5 h-3.5 fill-white dark:fill-black"
        >
...
function SectionHeader({
  ...
  return (
    <div className="flex justify-between items-center mb-5">
      <h3 className="font-bold text-text-light dark:text-text-dark text-base">
        {title}
      </h3>
      <button
        type="button"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-600 via-blue-700 to-indigo-800 p-8 text-center text-white">
        ...
        <div className="relative flex flex-col items-center">
          ...
          <h2 className="text-3xl font-extrabold mb-1 drop-shadow-lg">
            {personal.fullName || "Ready for"}{" "}
            <span className="text-blue-200">
              {personal.fullName ? "👋" : "Launch!"}
            </span>
          </h2>
          {experience.profileHeadline && (
            <p className="text-blue-100 text-sm mt-1 mb-2">
              {experience.profileHeadline}
            </p>
          )}
          <p className="text-blue-100/80 text-xs max-w-md mx-auto">
            Your profile is complete. Our AI is already finding the best matches
            for you.
          </p>
        </div>
      </div>
        ...
        <div className="flex items-center gap-4">
          {personal.profilePictureUrl ? (
            ...
            <div className="w-14 h-14 rounded-full bg-gradient-to-br from-blue-100 to-indigo-200 dark:from-gray-700 dark:to-gray-600 border-2 border-dashed border-gray-300 dark:border-gray-600 flex items-center justify-center flex-shrink-0">
              <span className="material-icons text-subtext-light text-2xl">
                person
              </span>
            </div>
          )}
          <div>
            <p className="font-bold text-text-light dark:text-text-dark">
              {personal.fullName || (
                <span className="text-subtext-light">Not set</span>
              )}
            </p>
            <p className="text-xs text-subtext-light dark:text-subtext-dark mt-0.5">
              {CAREER_GOAL_LABELS[personal.careerGoal] ??
      ...
      <div className="bg-white dark:bg-surface-dark border border-gray-200 dark:border-gray-800 rounded-xl p-6 shadow-sm space-y-5">
        ...
        {/* Profile headline */}
        <div>
          <p className="text-[10px] font-bold uppercase tracking-wider text-subtext-light dark:text-subtext-dark mb-1">
            Profile Headline
          </p>
          {experience.profileHeadline ? (
          ...
          {experience.education.length === 0 ? (
            ...
            <div className="space-y-2">
              {experience.education.map((edu) => (
                <div
                  key={edu.id}
                  className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 dark:bg-gray-800/60"
                >
                  <span className="material-icons text-primary text-[18px] mt-0.5">
                    school
                  </span>
                  <div>
                    <p className="text-sm font-semibold text-text-light dark:text-text-dark">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {experience.certifications.map((cert) => (
                <span
                  key={cert.id}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-xs font-semibold text-green-700 dark:text-green-300"
                >
            ...
            <div className="space-y-1">
              {experience.publications.map((pub) => (
                <div key={pub.id} className="flex items-center gap-2 text-sm">
                  ...
                  {pub.publisher && (
                    <span className="text-subtext-light dark:text-subtext-dark text-xs">
                      — {pub.publisher}
                    </span>
                  )}
                </div>
        ...
        <div>
          ...
          {jobLevel.careerLevel ? (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-primary/10 text-primary text-xs font-semibold">
              {CAREER_LEVEL_LABELS[jobLevel.careerLevel] ??
                jobLevel.careerLevel}
            </span>
          ) : (
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {(jobLevel.targetIndustries ?? []).map((ind) => (
                <span
                  key={ind}
                  className="px-3 py-1.5 rounded-full border border-gray-200 dark:border-gray-700 bg-slate-50 dark:bg-gray-800 text-xs font-semibold text-text-light dark:text-text-dark"
                >
                  {ind}
                </span>
              ))}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {(jobLevel.targetRoles ?? []).map((role) => (
                <span
                  key={role}
                  className="px-3 py-1.5 rounded-full border border-primary/40 bg-primary/5 text-primary text-xs font-semibold"
                >
                  {role}
                </span>
              ))}
          ...
          {skills.techSkills.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2">
              {skills.techSkills.map((s) => (
                <span
                  key={s}
                  className="px-3 py-1.5 bg-primary/10 text-primary dark:bg-primary/20 rounded-full text-xs font-semibold"
                >
                  {s}
                </span>
              ))}
